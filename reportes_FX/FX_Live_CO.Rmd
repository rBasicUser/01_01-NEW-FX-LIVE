...
p_top <- ggplot(data_60m, aes(x = Fecha)) +
  ...
  geom_line(aes(y = SMA200), color = "#967D00", linewidth = 0.9, alpha = 0.9) +
  labs(
...
p_bottom <- ggplot(data_24m, aes(x = Fecha)) +
  ...
  geom_ribbon(aes(ymin = Lower20, ymax = Higher20), alpha = 0.15, fill = "#FFD609") +
  geom_ribbon(aes(ymin = Lower50, ymax = Higher50), alpha = 0.10, fill = "#CCAA00") +
  geom_line(aes(y = SMA50), color = "#FFD609", linewidth = 0.9) +
  geom_line(aes(y = SMA20), color = "#CCAA00", linewidth = 0.9) +
  labs(
